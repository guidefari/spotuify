...
struct AppShell: View {
    ...
    var body: some View {
        ...
        // The window-level `ThemedView` (in `SpotuifyApp`) already applies the
        // preferred color scheme and the tint, so `AppShell` only needs to
        // forward `ArtworkTheme` to its descendants.
        .environment(theme)
    ...
    private var updateBannerView: some View {
        if autoCheckUpdates, model.banner == nil, let update = model.availableUpdate {
            ...
            .padding(.vertical, 10)
            .background(.thinMaterial, in: Capsule())
            .overlay(Capsule().strokeBorder(AlbumStageTokens.default.stroke))
            .shadow(color: ShadowTokens.default.medium, radius: 8, y: 2)
            .padding(.top, 10)
            .frame(maxWidth: 520)
            .transition(.move(edge: .top).combined(with: .opacity))
        }
    ...
    private var toastView: some View {
        if let toast = model.toast {
            ...
            .background(.thinMaterial, in: Capsule())
            .overlay(Capsule().strokeBorder(AlbumStageTokens.default.stroke))
            .shadow(color: ShadowTokens.default.medium, radius: 10, y: 3)
            .padding(.bottom, 112)
