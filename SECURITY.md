...
Do not paste access tokens, refresh tokens, Keychain exports, daemon logs, or bug-report bundles into public issues. If a public issue is the only available channel, describe the impact and reproduction shape without secrets so the maintainer can move the report to a private channel.
...
`spotuify` stores long-lived Spotify credentials in OS credential storage where available. The live Web API bearer is minted by the daemon and is treated as a secret. Commands that reveal secrets require explicit `--reveal-secret` flags.
