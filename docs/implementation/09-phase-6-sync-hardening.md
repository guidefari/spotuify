...
- Persist refresh tokens via Phase 11's per-platform keyring (not plaintext JSON).
- Refresh proactively at `expires_at - 60s`, not on 401. 401 handling stays as a safety net.
- Token-refresh race: a single `tokio::sync::Mutex<TokenState>` serializes refresh; concurrent requests await the in-flight refresh rather than starting their own.
